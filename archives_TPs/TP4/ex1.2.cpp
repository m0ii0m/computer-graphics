...
const GLfloat quad_vertices[] = {
    ...
    400.0f, 300.0f, 0.0f, // bottom-right
    300.0f, 300.0f, 0.0f  // bottom-left
};

const GLushort quad_elements[] = {
    0, 1, 2,  2, 3, 0
};
...
bool Initialise()
{
    ...
    // Initialisation du carré 2D (VAO2)
...
void Render()
{
    ...
    // --- Rendu 2D ---
    // Désactiver le depth test et culling pour dessiner par-dessus tout
    glDisable(GL_DEPTH_TEST);
    glDisable(GL_CULL_FACE);

    // Projection Orthographique 2D (0, 0 en haut à gauche)
    Matrix4 proj2D = Orthographic(0.0f, (float)width, (float)height, 0.0f, -1.0f, 1.0f);
    
    Matrix4 identity2D;
    Matrix4 transform2D = Multiply(proj2D, identity2D);

    glUniformMatrix4fv(loc_transform, 1, GL_FALSE, transform2D.m);
