...
bool Initialise()
{
    ...
    for (int i = 0; i < 12; ++i) {
        int offset = i * 9;
        Vector3D p0 = { g_cube_vertices2[offset+0], g_cube_vertices2[offset+1], g_cube_vertices2[offset+2] };
        Vector3D p1 = { g_cube_vertices2[offset+3], g_cube_vertices2[offset+4], g_cube_vertices2[offset+5] };
        Vector3D p2 = { g_cube_vertices2[offset+6], g_cube_vertices2[offset+7], g_cube_vertices2[offset+8] };
        
        Vector3D u = Subtract(p1, p0);
        Vector3D v = Subtract(p2, p0);
        Vector3D normal = Normalize(CrossProduct(u, v));
        
        cube_vertices_with_normals[i*3+0] = { p0, normal };
        cube_vertices_with_normals[i*3+1] = { p1, normal };
        cube_vertices_with_normals[i*3+2] = { p2, normal };
    }
