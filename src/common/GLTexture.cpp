#include "GLTexture.h"

#define STB_IMAGE_IMPLEMENTATION
#include "stb_image.h"

#include <cmath>
#include <algorithm>

GLuint LoadTexture(const char* filename) {
    int width, height, channels;
    stbi_set_flip_vertically_on_load(true);
    unsigned char* data = stbi_load(filename, &width, &height, &channels, 4);
    if (!data) {
        return 0;
    }

    GLuint texID;
    glGenTextures(1, &texID);
    glBindTexture(GL_TEXTURE_2D, texID);

    int mipLevels = 1 + std::floor(std::log2(std::max(width, height)));

    // Utilisation de glTexStorage2D (Immutable texture)
    glTexStorage2D(GL_TEXTURE_2D, mipLevels, GL_SRGB8_ALPHA8, width, height);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, width, height, GL_RGBA, GL_UNSIGNED_BYTE, data);
    glGenerateMipmap(GL_TEXTURE_2D);

    stbi_image_free(data);
    glBindTexture(GL_TEXTURE_2D, 0);
    return texID;
}

GLuint CreateGridTexture() {
    const int size = 64;
    unsigned char data[size * size * 4];
    for (int y = 0; y < size; y++) {
        for (int x = 0; x < size; x++) {
            int i = (y * size + x) * 4;
            // Motif de damier orangé
            unsigned char color = ((x / 8 + y / 8) % 2 == 0) ? 255 : 100;
            data[i+0] = color;       // R
            data[i+1] = color / 2;   // G
            data[i+2] = 50;          // B
            data[i+3] = 255;         // A
        }
    }

    GLuint texID;
    glGenTextures(1, &texID);
    glBindTexture(GL_TEXTURE_2D, texID);
    
    int mipLevels = 1 + std::floor(std::log2(size));
    glTexStorage2D(GL_TEXTURE_2D, mipLevels, GL_RGBA8, size, size);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, size, size, GL_RGBA, GL_UNSIGNED_BYTE, data);
    glGenerateMipmap(GL_TEXTURE_2D);
    
    glBindTexture(GL_TEXTURE_2D, 0);
    return texID;
}

GLuint LoadCubemap(const char* faces[6]) {
    GLuint texID;
    glGenTextures(1, &texID);
    glBindTexture(GL_TEXTURE_CUBE_MAP, texID);

    stbi_set_flip_vertically_on_load(false);
    
    // Pour un cubemap, il faut spécifier la taille à partir de la première face
    int width, height, channels;
    unsigned char* data = stbi_load(faces[0], &width, &height, &channels, 4);
    if (data) {
        int mipLevels = 1;
        glTexStorage2D(GL_TEXTURE_CUBE_MAP, mipLevels, GL_SRGB8_ALPHA8, width, height);
        glTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X, 0, 0, 0, width, height, GL_RGBA, GL_UNSIGNED_BYTE, data);
        stbi_image_free(data);
    }
    
    // Charger les 5 autres faces
    for (int i = 1; i < 6; i++) {
        data = stbi_load(faces[i], &width, &height, &channels, 4);
        if (data) {
            glTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, 0, 0,
                         width, height, GL_RGBA, GL_UNSIGNED_BYTE, data);
            stbi_image_free(data);
        }
    }
    stbi_set_flip_vertically_on_load(true);

    glBindTexture(GL_TEXTURE_CUBE_MAP, 0);
    return texID;
}
