...
void Render()
{
    ...
    // Rotation en 3D (sur X et Y) pour voir la profondeur s'appliquer
    Matrix4 rotX = RotateX(time * 2.0f);
    Matrix4 rotY = RotateY(time * 1.5f);
    Matrix4 rot = Multiply(rotY, rotX);
    
    // On le fait osciller en profondeur (entre -1.5 et -5.5) pour voir l'effet d'éloignement
    float zPos = -3.5f + 2.0f * std::sin(time * 1.0f);
    Matrix4 trans = Translate(0.5f * std::cos(time), 0.5f * std::sin(time), zPos);
